      ...
      <tbody>
        ...
        <tr class="row1" data-id="{{$cat->id}}">
          ...
          <td>
            ...
            <img class="margin-right-15 width-hight" align="left" src="{{url('images/category/'.$cat->image)}}"
              title="{{ $cat->title }}">

            @else
            <img class="margin-right-15" align="left" title="{{ $cat->title }}"
          ...
          <td>
            ...
            <form method="POST" action="{{ route('cat.quick.update',$cat->id) }}">
              ...
              <button @if(env('DEMO_LOCK')==0) type="submit" @else title="This operation is disabled in demo !"
                disabled="" @endif class="btn btn-xs {{ $cat->status ==1 ? 'btn-success' : 'btn-danger' }}">
          ...
          <td>
            ...
            <form method="POST" action="{{ route('cat.featured.quick.update',$cat->id) }}">
              {{ csrf_field() }}
              <button @if(env('DEMO_LOCK')==0) type="submit" @else title="This operation is disabled in demo !"
                disabled="" @endif class="btn btn-xs {{ $cat->featured ==1 ? 'btn-success' : 'btn-danger' }}">
              ...
              <li class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-right">
                  ...
                  <li role="presentation"><a role="menuitem" tabindex="-1"
                      href="{{url('admin/category/'.$cat->id.'/edit')}}"><i class="fa fa-pencil-square-o"
                        aria-hidden="true"></i>Edit Category</a></li>
                  @endcan
                  @can('category.delete')
                  <li role="presentation" class="divider"></li>
                  <li role="presentation">
                    <a @if(env('DEMO_LOCK')==0) data-toggle="modal" href="#{{ $cat->id }}deletecat" @else disabled

...
@section("body")
 ...
 <form action="{{ route('coupan.update',$coupan->id) }}" method="POST">
    ...
    <div class="box-body">
        ...
        <div class="form-group">
          <label>Linked to: <span class="required">*</span></label>
            ...
            <select required="" name="link_by" id="link_by" class="form-control">
              ...
              <option {{ $coupan->link_by == 'simple_product' ? "selected" : ""}} value="simple_product">Link By Simple Product</option>
        ...
        <div id="simpleprobox" class="form-group {{ $coupan->link_by == 'simple_product' ? "" : 'display-none' }}">
          <label>Select Simple Product: <span class="required">*</span> </label>
          <br>
          <select id="simple_pro_id" name="simple_pro_id" class="select2 form-control">
              @foreach(App\SimpleProduct::where('type','!=','ex_product')->where('status','1')->get() as $sproduct)
               
                <option {{ $coupan['simple_pro_id'] == $sproduct['id'] ? "selected" : "" }} value="{{ $sproduct->id }}">{{ $sproduct['product_name'] }}</option>

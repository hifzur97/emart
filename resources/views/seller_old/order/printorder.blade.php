	...
	<div class="container">
		...
		<table class="table table-striped">
			...
			<tbody>
				<tr>
					<td>
						<p><b>Total Qty:</b> {{ $sellerorders->sum('qty') }}</p>
						</p>
						<p><b>Order Total: <i class="{{ $order->paid_in }}"></i>{{ round($total,2) }}</b>
						</p>
		...
		<table class="font-size11 font-weight table table-striped table-bordered">
			...
			<tbody>
				...
				<tr>
					...
					<td></td>
					<td><b>Subtotal</b></td>
					<td><b><i class="{{ $invoice->order->paid_in }}"></i>{{ round($total - $hc - $giftcharge,2) }}</b>
					</td>

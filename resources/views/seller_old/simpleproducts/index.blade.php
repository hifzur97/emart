...
<div class="box">
    ...
    @if(env('ENABLE_SELLER_SUBS_SYSTEM') == 1 && getPlanStatus() == 1)
         ...
         @if((auth()->user()->products()->count() + auth()->user()->store->simple_products->count()) == auth()->user()->activeSubscription->plan->product_create/2)
         <br>
         ...
         @elseif((auth()->user()->products()->count() + auth()->user()->store->simple_products->count()) == auth()->user()->activeSubscription->plan->product_create)
         <br>
         <div class="callout callout-danger">
    ...
    <div class="box-header with-border">
        ...
        @endcan --}}
            ...
            <a title="Import products" href="{{ route('seller.import.product') }}" class="pull-right btn btn-md bg-olive"> {{__("Import Products")}} </a>

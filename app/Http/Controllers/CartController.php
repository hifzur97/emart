...
class CartController extends Controller
{
    ...
    public function create_cart()
    {
        ...
        if (Auth::check()) {
             ...
             $cart_table = auth()->user()->cart()->with(['simple_product','product','product.reviews','variant','product.shippingmethod'])->whereHas('product',function($query){
                return $query->where('status','1');
            })->whereHas('variant')->orWhereHas('simple_product')->get();
        ...
        if (!empty($user_id)) {
            ...
            if($ca->simple_product){
                ...
                $cart_table = Cart::where('simple_pro_id', $ca->simple_pro_id)
                    ...
                    'qty' => $days,
                    'tax_amount' => $ca->simple_product->tax_rate * $days,
                    'variant_id' => NULL,
                    'price_total' => $pricetotal1,
                    'semi_total' => $offertotal,
                ));
    ...
    public function simpeproductincart(Request $request,$pro_id,$price,$offerprice){
        ...
        $cart->tax_amount = $product->tax_rate * $qty;

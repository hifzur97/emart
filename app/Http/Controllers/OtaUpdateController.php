...
class OtaUpdateController extends Controller
{
    ...
    public function checkforupate(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $repo = @file_get_contents('https://raw.githubusercontent.com/thegr8dev/emart/' . $current_version . '/' . $new_version . '.json');
    ...
    public function mergeQuickupdate(Request $request)
    {
        ...
        $file = @file_get_contents('https://raw.githubusercontent.com/thegr8dev/emart/' . config('app.version') . '/' . $request->filename);

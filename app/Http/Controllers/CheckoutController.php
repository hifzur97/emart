...
class CheckoutController extends Controller
{
    ...
    public function getBillingView()
    {
        ...
        if (auth()->check()) {
            ...
            if ($genrals_settings->cart_amount != 0 && $genrals_settings->cart_amount != '') {
                ...
                if (getcarttotal() * $conversion_rate >= $genrals_settings->cart_amount * $conversion_rate) {
    ...
    public function index(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            $cart_table = auth()->user()->cart()->with(['product', 'variant', 'product.shippingmethod', 'simple_product'])->whereHas('product', function ($query) {
